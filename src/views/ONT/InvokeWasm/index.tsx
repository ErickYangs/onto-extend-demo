...
const OntInvokeWasm: React.FC = () => {
  const [sccall, setSccall] = useState<string>('')
  const [sccallread, setSccallread] = useState<string>('')
  const [sccallreadarray, setSccallreadarray] = useState<string>('')
  const [sccallreadmap, setSccallreadmap] = useState<string>('')
  ...
  async function onScCall(values: any) {
    ...
    try {
      const result = await client.api.smartContract.invokeWasm({
        scriptHash,
        operation,
        args,
        gasPrice,
      ...
      setSccall(JSON.stringify(result))
    } catch (e) {
  ...
  async function onScCallRead(values: any) {
    ...
    try {
      const result = await client.api.smartContract.invokeWasmRead({
        scriptHash,
        operation,
        args,
      })
      ...
      setSccallread(JSON.stringify(result))
    } catch (e) {
  ...
  async function onScCallReadArray(values: any) {
    ...
    try {
      ...
      // tslint:disable-next-line:no-console
      console.log('onScCallRead finished, result:' + JSON.stringify(result))
      setSccallreadarray(JSON.stringify(result))
    } catch (e) {
      alert('onScCallRead canceled')
      // tslint:disable-next-line:no-console
      console.log('onScCallRead error:', e)
  ...
  return (
    ...
    >
      <h2>invokeWasm</h2>
      <h2>ScCall</h2>
      <div className={'left-grid-lay'}>
        <Form
          initialValues={{
            contract: '60783b25b71bb97bac26022843dfa623baf6290d',
            method: 'transfer',
            gasPrice: '3000',
            gasLimit: '100000000',
            parameters: [
              { type: 'Address', value: 'AcfgBpnGADm9cTF6CiNbJeeUiLfaPuFGRt' },
              { type: 'Address', value: 'AJpcHUpGZx638SwE45TSTnwzPK8Cfc1Ztt' },
          ...
          render={({
            ...
            <form onSubmit={handleSubmit}>
              ...
              <button
                type="button"
                onClick={() =>
                  push('parameters', { type: 'Integer', value: '' })
                }
              >
                Add Parameter
              </button>
        ...
        />
        <div className={'result-cell'}>Result: {sccall}</div>
      </div>
      ...
      <div className={'left-grid-lay'}>
        ...
        />
        <div className={'result-cell'}>Result: {sccallread}</div>
      </div>
      ...
      <div className={'left-grid-lay'}>
        ...
        />
        <div className={'result-cell'}>Result: {sccallreadarray}</div>
      </div>
      ...
      <div className={'left-grid-lay'}>
        ...
        />
        <div className={'result-cell'}>Result: {sccallreadmap}</div>
      </div>
